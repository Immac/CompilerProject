		...
		switch (state) {
			...
			case LexicalState::Id:
				if (isalnum(this->_currentSymbol.Value)) {
					...
					return Token(lexemeLowercase, type, this->_row, this->_column);
				}
				break;
			case LexicalState::IntegerLiteralDecimal:
				if (isdigit(this->_currentSymbol.Value)) {
					...
					return Token(lexeme, TokenClass::IntegerLiteralDecimal, this->_row, this->_column);
				}
				break;
			case LexicalState::IntegerLiteralHexadecimalSingle:
				if (std::find(ValidHexValues.begin(), ValidHexValues.end(), this->_currentSymbol.Value) !=
					ValidHexValues.end()) {
			...
			case LexicalState::IntegerLiteralHexadecimalDigits:
				if (Util::Contains<char>(ValidHexValues, tolower(this->_currentSymbol.Value))) {
					...
					return Token(Util::ToLower(lexeme), TokenClass::IntegerLiteralHexadecimal, this->_row,
								 this->_column);
				}
				break;
			case LexicalState::IntegerLiteralOctal:
				if (std::find(ValidOctValues.begin(), ValidOctValues.end(), this->_currentSymbol.Value) !=
					ValidOctValues.end()) {
					state = LexicalState::IntegerLiteralOctal;
					lexeme += this->_currentSymbol.Value;
					this->ConsumeSymbol();
					...
					return Token(lexeme, TokenClass::IntegerLiteralOctal, this->_row, this->_column);
				}
				else {
					ThrowLexicalError("Expected a hexadecimal digit"); // TODO: got <symbol>, lexeme: lexeme
				}
			...
			case LexicalState::CharLiteral:
				...
				if (isdigit(_currentSymbol.Value)) {
					...
					return Token(lexeme, TokenClass::CharLiteral, this->_row, this->_column);
				}
				else {
			...
			case LexicalState::StringLiteralClosingDoubleQuote:
				if (this->_currentSymbol.Value == '\"') {
					...
					return Token(lexeme, TokenClass::StringLiteralDoubleQuote, this->_row, this->_column);
				}
				break;
			...
			case LexicalState::StringLiteralClosingSingleQuote:
				if (this->_currentSymbol.Value == '\'') {
					...
					return Token(lexeme, TokenClass::StringLiteralSingleQuote, this->_row, this->_column);
				}
				break;
			...
			case LexicalState::UnambiguousPunctuationState:
				if (lexeme == OpenCommentCurlyBrace) {
					...
					return Token(lexeme, Punctuation.at(lexeme), this->_row, this->_column);
				}
				break;
			...
			case LexicalState::EndOfFile:
				...
				return Token(lexeme, TokenClass::EndOfFile, this->_row, this->_column);
		}
	}
}

Token Lexer::HtmlToken() {
	std::string lexeme = "";
	auto state = LexicalState::Initial;
	this->UpdatePosition();
	while (true) {
		...
		switch (state) {
			...
			case LexicalState::TryForHtml: {
				...
				ConsumeSymbol();
				if (isClosingTag) {
					return Token(Util::ToLower(lexeme), TokenClass::HtmlCloseTag, this->_row, this->_column);
				}
				else {
					return Token(Util::ToLower(lexeme), TokenClass::HtmlOpenTag, this->_row, this->_column);
				}
			}
			case LexicalState::EndOfFile:
				return Token(lexeme, TokenClass::EndOfFile, this->_row, this->_column);
			case LexicalState::HtmlContent:
				...
				if (lastTwo == "<%") {
					this->SetAnalysisMode(AnalysisMode::Pascal);
					return Token(lexeme.substr(0, lexemeSizeMinusTwo), TokenClass::HtmlContent, this->_row,
								 this->_column);
				}
				else if (this->_currentSymbol.Value != '<'
					...
					return Token(lexeme, TokenClass::HtmlContent, this->_row, this->_column);
				}
				break;
		}
	}
